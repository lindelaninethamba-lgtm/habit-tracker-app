...
def filter_by_periodicity(habits: list, period: str) -> list: #returns filtered habits
    def matches_period(habit): #matches every habit with matching periodicity call
        return habit.periodicity == period
    return list(filter(matches_period, habits)) #returns list of habits with same periodicity as call
...
def longest_streak_all(habits: list, all_logs: list) -> tuple: #returns longest streak overall in a tuple
    if not habits:
        return None, 0 #return zero if habit list is empty

    def get_streak(habit): #nested function to returnstreak number
    ...
    def make_streak_tuple(habit): #makes streak a tuple
        return (habit, get_streak(habit)) #calls habit for which streak it belongs 

    def compare_streaks(a, b): #compares streaks
